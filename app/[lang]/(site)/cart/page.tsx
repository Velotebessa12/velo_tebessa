...
<div className="mt-2 flex flex-col gap-1">
  ...
  <div className="flex items-center gap-2 flex-wrap">
    ...
    {/* Old price */}
    {item.regularPrice && item.finalPrice < item.regularPrice && (
      <span className="text-xs text-gray-400 line-through">
        {item.regularPrice.toLocaleString()} DA
      </span>
    )}

    {/* Final price */}
    <span className="text-base sm:text-lg font-bold text-teal-600">
      {item.finalPrice.toLocaleString()} DA
    </span>
  ...
  <p className="text-sm font-bold text-gray-800">
    {(item.finalPrice * item.quantity).toLocaleString()} DA
  </p>
