...
export default function PurchaseInvoicesPage() {
  ...
  useEffect(() => {
    ...
    setFilteredProducts(
      products.filter((p: any) =>
        getTranslations(p.translations, lang, "name").toLowerCase().includes(q),
      ),
    );
 ...
 function openEditPopup(invoiceData: any) {
  ...
  const mappedRows = invoiceData.items.map((item: any) => ({
    id: item.id,
  ...
  const updateRow = (id: number, field: keyof any, value: string | number) => {
    setRows((prev) =>
      prev.map((r) => {
        if (r.id !== id) return r;
 ...
 const editInvoice = async (id: string, data: Record<string, any>) => {
  try {
...
const cancelInvoice = async (id: string) => {
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.error || "Error cancelling invoice");
    }
  ...
  >
    ...
    Calculer le PMP
              ...
              <button
                onClick={() =>
                  router.push(`/${lang}/admin/products/new?redirect=invoices`)
                }
