...
import { Edit, Eye, Pencil, Plus, Search, Trash2 } from "lucide-react";
...
const Page = () => {
  ...
  const [type, setType] = useState<"PRODUCT" | "ADDITION" | "PACK">("PRODUCT");
  ...
  useEffect(() => {
    async function fetchItems() {
      try {
        ...
        if (type === "PRODUCT" || type === "ADDITION") {
          url = `/api/products/get-products?type=${type}`;
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-4 md:p-5 hover:shadow-md transition-shadow">
          <p className="text-[10px] sm:text-xs md:text-sm font-medium text-gray-600 mb-1">
            Total {type === "PRODUCT" ? "Products"
            : type === "ADDITION" ? "Additions"
          : "Packs"}
          </p>
  ...
  {packs.map((pack) => (
    ...
    >
      ...
      <div className="relative flex-shrink-0 w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16">
        <Image
          src={pack.imageUrl || "/placeholder.png"}
          alt=""
          fill
      ...
      {/* Name */}
      <div
        onClick={() => router.push(`/${lang}/packs/${pack.id}`)}
      ...
      <div className="flex-shrink-0 text-xs sm:text-sm font-semibold text-gray-900 tabular-nums">
        {pack.promoPrice !== null && pack.promoPrice !== undefined
          ? `${pack.promoPrice.toLocaleString()} DA`
          : pack.regularPrice !== null && pack.regularPrice !== undefined
          ? `${pack.regularPrice.toLocaleString()} DA`
          : "—"}
      </div>

      {/* Items count */}
      <div className="hidden sm:flex flex-shrink-0 items-center px-2 sm:px-3 py-1 rounded-lg text-[10px] sm:text-xs font-semibold bg-blue-100 text-blue-700">
        {pack.items?.length || 0}
        <span className="hidden md:inline ml-1">produits</span>
      </div>
      ...
      <div className="flex-shrink-0 flex items-center gap-0.5 sm:gap-1">
        ...
        <button
          onClick={() =>
            router.push(`/${lang}/admin/packs/new?update=${pack.id}`)
          }
          ...
          >
            ...
            <div className="flex-shrink-0 flex items-center gap-0.5 sm:gap-1">
              ...
              <button
                onClick={() =>
                  router.push(
                    `/${lang}/admin/products/new?update=${product.id}`,
                  )
                }
                className="p-1.5 sm:p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors"
                title="Modifier"
